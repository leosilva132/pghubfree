...
const fieldValue = document.querySelector("#field-src");
...
const Values = {
    getValue(event){
        if(fieldValue.value.length > 0){
            ...
            ClearValues.clearHTML()
            Users.getUser(fieldValue.value)
            getRepos.FetchRepos(fieldValue.value)
        } else {
            alert("Por favor preencha o campo com nume de usuário")
        }
    }
}
